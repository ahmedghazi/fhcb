...
const EVENTS_QUERY = groq`*[_type == "event" && !(_id in path("drafts.**"))] | order(coalesce(dates[0].du, _createdAt) asc) {
  ${cardRefEvent}
}`;

const PRODUCTS_QUERY = groq`*[_type == "product" && !(_id in path("drafts.**")) && references(*[_type == "tag" && slug.current == "livre-du-mois"]._id)] | order(_createdAt desc) {
  ${cardRefProduct}
}`;
